...
`run.ps1` / `run.sh` read `config.json`, start the llama.cpp router (`llama-server --models-preset <models.ini> --models-max <router_models_max, default 1> --offline --host <router_host> --port <router_port> --metrics`, plus `--api-key` if one is set) if it is not already listening, start the dashboard backend (`backend/server.py`, bound to `panel_host:panel_port`) if it is not already listening, then open `http://127.0.0.1:<panel_port>/` in your browser. Both scripts are safe to run repeatedly — each checks whether its port is already in use before starting anything.
