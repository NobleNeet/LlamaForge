...
4. To save a set of knobs for later on that same model, click **Save current +** in the preset bar to name the model's current settings as a preset. Presets are model-scoped: each model row only shows and applies its own saved presets. Delete a preset with the `×` on its chip.
   - To make a preset that model's **default**, click the ◉/○ dot on its chip to *bind* it (`POST /api/presets/bind`). Binding writes the preset's knobs into that model's section now. The bound chip is highlighted; clicking the dot again unbinds and leaves the knobs in place. Re-saving a bound preset re-syncs that same model.
...
| Presets | `POST /api/presets/save` / `/apply` / `/delete` / `/bind` | Named knob sets stored per model in `config.json`'s `model_presets` key; applying one follows the same save + reload path as a manual edit. **Bind** records the pairing in `preset_bindings` and materializes the preset into that same model. |
