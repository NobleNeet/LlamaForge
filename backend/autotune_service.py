...
class AutoTuneService:
    ...
    def _worker(self, run_id, path, snapshot, hardware_fp, token):
        try:
            ...
            AutoTuneOrchestrator(self.store, self.runner_factory()).run(run_id, strategy, rules,
                tuple(item.execution_environment for item in prepared), BenchmarkTarget(path, self.fingerprint(path).value),
                3, 120, token, tuple(prepared))
        except Exception:
            try:
                self.store.acquire(run_id); self.store.finish(run_id, "failed")
            except Exception:
                pass
        finally:
            with self._registry_lock: self._registry.pop(run_id, None)
    ...
    def reconcile_startup(self):
        return self.store.reconcile_interrupted_runs()

    @staticmethod
    def _summary(manifest):
        return {"run_id": manifest.get("run_id"), "status": manifest.get("status"), "progress": manifest.get("progress"),
                "model": manifest.get("target", {}).get("model_name"), "created_at": manifest.get("started_at"),
