...
class TestPresets(unittest.TestCase):
    ...
    def test_blank_values_dropped(self):
        ...
        self.assertEqual(config.get_presets("m")["fast"], {"temp": "0.7"})

    def test_overwrite_existing(self):
        config.save_preset("m", "x", {"temp": "0.1"})
        config.save_preset("m", "x", {"temp": "0.9"})
        self.assertEqual(config.get_presets("m")["x"], {"temp": "0.9"})
    ...
    def test_survives_non_dict_presets_field(self):
        ...
        self.assertEqual(config.get_presets("m")["ok"], {"temp": "0.5"})

    def test_legacy_global_presets_are_visible_until_model_writes(self):
        config.save({**config.load(), "presets": {"legacy": {"temp": "0.4"}}})
...
class ExplicitGpuLayerPresetTest(unittest.TestCase):
    def test_partial_and_cpu_values_survive_preset_save(self):
        from unittest import mock
        with tempfile.TemporaryDirectory() as tmp, mock.patch.object(config, 'CONFIG', os.path.join(tmp, 'config.json')):
            for value in ('0', '17', '49'):
                config.save_preset('m', 'balanced', {'n-gpu-layers': value})
                self.assertEqual(config.get_presets('m')['balanced']['n-gpu-layers'], value)
